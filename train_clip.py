"""
Обучение и оценка multitask CLIP на DeepFashion (img_highres).

Пример:
  python train_clip.py --output-dir runs/clip --run-name ViT-B-32_base
"""
...
from photoclassifier.models.clip_multitask import MultiTaskCLIP
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("runs/clip"))
    parser.add_argument("--run-name", type=str, default=None)
    parser.add_argument("--overwrite-output", action="store_true")
    parser.add_argument("--variant", choices=("ViT-B-32", "ViT-B-16", "ViT-L-14"), default="ViT-B-32")
    ...
    cfg = TrainConfig()
    cfg.backbone = f"clip_{args.variant}"
        ...
        json.dumps(
            {
                ...
                "architecture": "clip",
                "model_name": f"clip_{args.variant}",
            },
            ensure_ascii=False,
            indent=2,
        ),
    ...
    model = MultiTaskCLIP(num_type, num_color, num_style, model_name=args.variant, pretrained="laion2b_s34b_b79k").to(device)
