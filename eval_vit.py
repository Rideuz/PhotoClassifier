"""
Оценка чекпоинта ViT без обучения.
"""
...
def main() -> None:
    ...
    model_name = str(meta.get("model_name", "vit_b_16"))
    # Для ViT имя модели: vit_b_16 или vit_b_32. Нужно выделить "b_16" или "b_32"
    if model_name.startswith("vit_"):
        variant = model_name[4:]
    else:
        variant = "b_16"
    model = MultiTaskViT(num_type, num_color, num_style, variant=variant, pretrained=False)
