"""
Оценка чекпоинта CLIP без обучения.
"""
...
def main() -> None:
    ...
    model_name = str(meta.get("model_name", "clip_ViT-B-32"))
    # Для CLIP имя модели: clip_ViT-B-32 или clip_b_32. Нужно выделить "ViT-B-32" или "b_32"
    if model_name.startswith("clip_"):
        variant = model_name[4:]
    if model_name.startswith("clip_"):
        variant = model_name[5:]
    else:
        variant = "ViT-B-32"
    model = MultiTaskCLIP(num_type, num_color, num_style, model_name=variant, pretrained=None)
