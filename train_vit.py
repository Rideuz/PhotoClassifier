"""
Обучение и оценка multitask ViT на DeepFashion (img_highres).

Пример:
  python train_vit.py --output-dir runs/vit --run-name b_16_base
"""
...
from photoclassifier.models.vit_multitask import MultiTaskViT
...
def main() -> None:
    ...
    parser.add_argument("--variant", choices=("b_16", "b_32"), default="b_16")
    ...
    cfg = TrainConfig()
    cfg.backbone = f"vit_{args.variant}"
        ...
        json.dumps(
            {
                ...
                "architecture": "vit",
                "model_name": f"vit_{args.variant}",
            },
            ensure_ascii=False,
            indent=2,
        ),
    ...
    model = MultiTaskViT(num_type, num_color, num_style, variant=args.variant, pretrained=cfg.pretrained).to(device)
